...
import org.apache.pekko.actor.{Actor, ActorRef, ActorSystem, NoSerializationVerificationNeeded, PoisonPill, Props, ReceiveTimeout}
...
class PekkoSensorsSpec extends AnyFreeSpec with LazyLogging with Eventually with BeforeAndAfterAll {
  ...
  implicit val registry: CollectorRegistry = PekkoSensors.prometheusRegistry

  "Launch Pekko app, and ensure it works" - {
    ...
    "ensure prometheus JMX scraping is working" in {
      ...
      val blockingIo = system.dispatchers.lookup("pekko.actor.default-blocking-io-dispatcher")
      ...
      List("pekko_sensors_actor", "pekko_sensors_dispatchers").foreach(s => assert(content.exists(_.startsWith(s)), s"starts with $s"))
    }
    ...
    "ensure MANY actors are created and stopped, all accounted for" in {
      ...
      eventually {
        assertMetrics(
          _.startsWith("pekko_sensors_actor_active_actors_total{actor=\"MassProbe\""),
    ...
    trait PersistentScope {
      ...
      eventually {
        assertMetrics(
          _.startsWith(s"""pekko_sensors_actor_active_actors_total{actor="$actorName""""),
          _.endsWith(s" $actors.0")
        )
      }
      eventually {
