...
import org.apache.pekko.actor.{ActorSystem, ClassicActorSystemProvider, CoordinatedShutdown, ExtendedActorSystem, Extension, ExtensionId, ExtensionIdProvider, Props}
...
import nl.pragmasoft.pekko.sensors.actor.ClusterEventWatchActor
...
object PekkoSensors extends LazyLogging {

  private val config                    = ConfigFactory.load().getConfig("pekko.sensors")
 ...
 */
class PekkoSensorsExtension(system: ExtendedActorSystem) extends Extension with LazyLogging {

  val registry = PrometheusRegistry.defaultRegistry

  logger.info(s"Pekko Sensors extension has been activated: ${this.getClass.getName}")
  if (PekkoSensors.ClusterWatchEnabled)
...
object PekkoSensorsExtension extends ExtensionId[PekkoSensorsExtension] with ExtensionIdProvider {
  override def lookup: ExtensionId[_ <: Extension] = PekkoSensorsExtension
  override def createExtension(system: ExtendedActorSystem): PekkoSensorsExtension = {
    val extensionClass = ConfigFactory.load().getString("pekko.sensors.extension-class")
    Class.forName(extensionClass).getDeclaredConstructor(classOf[ExtendedActorSystem]).newInstance(system) match {
      case w: PekkoSensorsExtension => w
      case _                        => throw new IllegalArgumentException(s"Class $extensionClass must extend com.ing.bakery.baker.Watcher")
    }
  }
  override def get(system: ActorSystem): PekkoSensorsExtension                = super.get(system)
  override def get(system: ClassicActorSystemProvider): PekkoSensorsExtension = super.get(system)
}
...
object MetricOps {
  ...
  implicit class HistogramExtensions(val histogram: DistributionDataPoint) {
    def observeExecution[A](f: => A): A = {
      ...
      try f
      finally histogram.observe((System.nanoTime() - startNanos) / 1e6)
    }

    def observeEffect[E, S](eff: EffectBuilder[E, S]): EffectBuilder[E, S] = {
      val startNanos = System.nanoTime()
      eff.thenRun(_ => histogram.observe((System.nanoTime() - startNanos) / 1e6))
    }
  }
