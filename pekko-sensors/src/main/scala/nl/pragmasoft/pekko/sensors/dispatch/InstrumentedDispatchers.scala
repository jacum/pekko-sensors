...
object DispatcherInstrumentationWrapper {
  ...
  def meteredRun(id: String): InstrumentedRun = {
    ...
    val queue          = queueTime.labelValues(id)
    val run            = runTime.labelValues(id)
    val active         = activeThreads.labelValues(id)
  ...
  override def createExecutorServiceFactory(id: String, threadFactory: ThreadFactory): ExecutorServiceFactory = {
    ...
    new ExecutorServiceFactory {
      def createExecutorService: ExecutorService = {
        ...
        lazy val activeCount       = executorValue.labelValues(id, "activeCount")
        lazy val corePoolSize      = executorValue.labelValues(id, "corePoolSize")
        lazy val largestPoolSize   = executorValue.labelValues(id, "largestPoolSize")
        lazy val maximumPoolSize   = executorValue.labelValues(id, "maximumPoolSize")
        lazy val queueSize         = executorValue.labelValues(id, "queueSize")
        lazy val completedTasks    = executorValue.labelValues(id, "completedTasks")
        lazy val poolSize          = executorValue.labelValues(id, "poolSize")
        lazy val steals            = executorValue.labelValues(id, "steals")
        lazy val parallelism       = executorValue.labelValues(id, "parallelism")
        lazy val queuedSubmissions = executorValue.labelValues(id, "queuedSubmissions")
        lazy val queuedTasks       = executorValue.labelValues(id, "queuedTasks")
        lazy val runningThreads    = executorValue.labelValues(id, "runningThreads")
