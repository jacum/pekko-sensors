...
class EventsByPersistenceIdTest extends QueryTestSpec {
  ...
  it should "find events for actor with pid 'my-2'" in {
    ...
    withEventsByPersistenceId()("my-2", 0, Long.MaxValue) { tp =>
      ...
      tp.expectNext(ExpectNextTimeout, EventEnvelope(Sequence(4), "my-2", 4, "a-4", System.currentTimeMillis, None))
      tp.expectNext(ExpectNextTimeout, EventEnvelope(Sequence(5), "my-2", 5, "a-5", System.currentTimeMillis, None))
      tp.expectNext(ExpectNextTimeout, EventEnvelope(Sequence(6), "my-2", 6, "a-6", System.currentTimeMillis, None))
      tp.expectNoMessage(expectTime)
