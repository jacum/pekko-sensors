...
package nl.pragmasoft.pekko.persistence.inmemory.journal
...
import nl.pragmasoft.pekko.persistence.inmemory.extension.{InMemoryJournalStorage, StorageExtensionProvider}
...
class InMemoryAsyncWriteJournal(config: Config) extends AsyncWriteJournal {
  ...
  val log: LoggingAdapter           = Logging(system, classOf[InMemoryAsyncWriteJournal])
