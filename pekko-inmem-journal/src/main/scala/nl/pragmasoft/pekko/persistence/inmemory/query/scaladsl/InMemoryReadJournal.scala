...
package nl.pragmasoft.pekko.persistence.inmemory.query.scaladsl

import com.typesafe.config.Config
import nl.pragmasoft.pekko.persistence.inmemory.JournalEntry
import nl.pragmasoft.pekko.persistence.inmemory.extension.InMemoryJournalStorage
import nl.pragmasoft.pekko.persistence.inmemory.extension.InMemoryJournalStorage.PersistenceIds
import org.apache.pekko.NotUsed
import org.apache.pekko.actor.{ActorRef, ExtendedActorSystem}
import org.apache.pekko.event.{Logging, LoggingAdapter}
import org.apache.pekko.pattern.ask
import nl.pragmasoft.pekko.persistence.inmemory.util.UUIDs
...
class InMemoryReadJournal(config: Config, journal: ActorRef)(implicit val system: ExtendedActorSystem)
    extends ReadJournal
  ...
  override def currentPersistenceIds(): Source[String, NotUsed] =
    Source
      .future((journal ? PersistenceIds).mapTo[Set[String]])
      .mapConcat(identity)

  override def persistenceIds(): Source[String, NotUsed] =
    Source
      .repeat(0)
      .flatMapConcat(_ => Source.tick(refreshInterval, 10.milliseconds, 0).take(1).flatMapConcat(_ => currentPersistenceIds()))
      .mapConcat[String] {
  ...
  override def currentEventsByPersistenceId(persistenceId: String, fromSequenceNr: Long, toSequenceNr: Long): Source[EventEnvelope, NotUsed] =
    Source
      .future(
        (journal ? InMemoryJournalStorage.GetJournalEntriesExceptDeleted(persistenceId, fromSequenceNr, toSequenceNr, Long.MaxValue))
          .mapTo[List[JournalEntry]]
      )
      .mapConcat(identity)
      .via(deserialization)
      .map(repr => EventEnvelope(Offset.sequence(repr.sequenceNr), repr.persistenceId, repr.sequenceNr, repr.payload, System.currentTimeMillis, None))
    ...
    Source
      .unfoldAsync[Long, Seq[EventEnvelope]](Math.max(1, fromSequenceNr)) { (from: Long) =>
        ...
        from match {
          ...
          case _ =>
            Source
              .tick(refreshInterval, 10.milliseconds, 0)
              .take(1)
              .flatMapConcat(_ =>
  ...
  override def currentEventsByTag(tag: String, offset: Offset): Source[EventEnvelope, NotUsed] =
    Source
      .future(
        (journal ? InMemoryJournalStorage.EventsByTag(tag, offset))
          .mapTo[List[JournalEntry]]
      )
      .mapConcat(identity)
  ...
  override def eventsByTag(tag: String, offset: Offset): Source[EventEnvelope, NotUsed] =
    Source
      .unfoldAsync[Offset, Seq[EventEnvelope]](offset) { (from: Offset) =>
        def nextFromOffset(xs: Seq[EventEnvelope]): Offset =
          if (xs.isEmpty) from
          else
            xs.last.offset match {
  ...
  // ticker
  val ticker = Source.tick(refreshInterval, 10.milliseconds, 0).take(1)
  ...
  private def deserializationWithOffset(offset: Offset): Flow[JournalEntry, (Offset, PersistentRepr), NotUsed] =
    Flow[JournalEntry]
      .flatMapConcat(entry =>
