package org.apache.pekko.sensors.cassandra

import java.io.CharArrayWriter
import java.net.InetSocketAddress
import java.util.UUID

import org.apache.pekko.actor.{ActorLogging, ActorSystem, NoSerializationVerificationNeeded, Props}
...
class PekkoCassandraJmxMetricsSpec extends AnyFreeSpec with LazyLogging with Eventually with BeforeAndAfterAll {
  ...
  "Launch cassandra and pekko app, and ensure it works" ignore {
    ...
    "after some activity exports some jmx metrics" in {
      ...
      session.execute("create table org.apache.pekko.test (id int, a text, b text, primary key(id))")
      for (i <- 1 to 100)
        session
          .execute(
            insertInto("pekko", "test")
      ...
      mbs.queryMBeans(null, null).asScala.map(_.getObjectName).filter(_.toString.startsWith("pekko")).foreach(println)
